...
export abstract class BaseWordActionButtonComponent {
  ...
  onClick(): void {
    ...
    if (this.isReviewed) {
      // Skip confirmation for auto-accepted statuses when changing to reviewed good, accept, or context accept
      const currentStatus = this.wordWithStatus && this.wordWithStatus.status;
      const targetStatus = this.getTargetStatus();
      const isAutoAcceptedStatus = currentStatus === ReviewedWordStatus.AUTO_ACCEPTED_FROM_PUBLIC ||
        currentStatus === ReviewedWordStatus.AUTO_ACCEPTED_FROM_BIBLE;
      ...
      if (isAutoAcceptedStatus && isChangingToGoodStatus) {
