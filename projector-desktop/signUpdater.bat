@echo off
REM Sign updater.exe with code signing certificate
...
set TARGET_FILE=..\projector-desktop-updater\build\jpackage\updater\updater.exe
...
if not exist "%TARGET_FILE%" (
    ...
    echo Updater may not have been built. Skipping signing.
    exit /b 0
)

echo Signing updater.exe...
...
if errorlevel 1 (
    echo ERROR: Failed to sign updater.exe
    exit /b 1
)

echo.
echo SUCCESS: updater.exe has been signed.
echo.
